...
public class Application1 {
    public static void main(String[] args) {
        ...
        * BufferedReader/Writer : 버퍼 공간을 이용하여 데이터를 쌓아 두었다가 입출력하여
        *                         입출력 횟수를 줄이고 성능을 향상 (한번에 입출력)
        * */
        BufferedWriter writer = null;

        try {
            /* 보조 스트림 객체 생성 시에는 생성자의 인자로 연결 됨 기반 스트림을 반드시 전달한다. */
            ...
            * 가득 차지 않은 경우 flush() 로 내보내기를 해야 출력이 이루어진다. */
            ...
            if(writer != null){
                try {
                    /* close() 호출하면 내부적으로 Flush 호출하고 난 뒤 자원을 반환한다. */
                    writer.close();
                } catch (IOException e) {
                    System.out.println(e.getMessage());
                }
        ...
        try(BufferedReader reader = new BufferedReader(new FileReader("testBuffered.txt"))) {
            ...
            * 버퍼의 한 줄을 읽어와서 문자열로 반환하여 더 이상 읽어올 값이 없을 경우 null 반환.
            *  */
            String temp;
